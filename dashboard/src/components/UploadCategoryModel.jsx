...
export default function UploadCategoryModel({ onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (dataResponse.success) {
        ...
        dispatch({ type: "add_category", payload: dataResponse.data });
        onSuccess();
        onClose();
      } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-1">
              <label
                htmlFor="icon_url"
                className="flex gap-2 items-center mb-2 text-lg font-medium text-gray-700 transition-colors cursor-pointer hover:text-gray-900"
              >
                <FcAddImage size={32} className="animate-bounce" />
                <span>Chọn ảnh</span>
              </label>
              <div className="mt-2">
                <div className="flex overflow-hidden justify-center items-center w-full h-24 bg-gray-50 rounded-xl border border-gray-300">
                  {data?.icon_url ? (
                    <img
                      src={data.icon_url}
          ...
          <button
            ...
            className={`w-full py-3 px-4 font-medium text-white rounded-xl transition-all duration-300 transform hover:scale-[1.02] disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none ${loading || loadingImage.icon || loadingImage.banner
                ? "bg-gray-400"
