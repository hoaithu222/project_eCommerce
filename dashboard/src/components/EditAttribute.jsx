...
export default function EditAttribute({ onClose, dataAttributeId, onSuccess }) {
  const [data, setData] = useState({
    id: Number(dataAttributeId.id),
    name: dataAttributeId.name,
    value: dataAttributeId.attribute_values?.map((av) => av.value) || [],
    category_id: dataAttributeId.categories?.map((c) => c.category_id) || [],
  });
  ...
  useEffect(() => {
    const initSelectedCategories = async () => {
      try {
        // Lấy thông tin chi tiết của các category đã chọn từ categories
        const selectedCats = categories.filter((cat) =>
          dataAttributeId.categories?.some((c) => c.category_id === cat.id)
        );
        setSelectedCategories(selectedCats);
      } catch (error) {
        console.error("Error initializing selected categories:", error);
      }
    };
  ...
  }, [categories, dataAttributeId.categories]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${SummaryApi.editAttributes.url}/${data.id}`,
        {
          method: SummaryApi.editAttributes.method,
          credentials: "include",
          headers: {
      ...
      if (dataResponse.success) {
        ...
        dispatch({ type: "update_attributes", payload: dataResponse.data });
        onSuccess();
        onClose();
      } else {
